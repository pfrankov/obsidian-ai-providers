...
export function preprocessContent(content: string): string {
    ...
    return (
        content
            ...
            .replace(/[ \t]+$/gm, '')
            // Remove trailing newlines from the entire content
            .replace(/\n+$/, '')
    );
}
...
function splitSectionWithLists(section: string): string[] {
    ...
    const finalChunks = result.currentChunk
        ? [...result.chunks, result.currentChunk.trim()]
        : result.chunks.map(chunk => chunk.trim());

    // Check if there are chunks that contain only a header
    // If yes, merge them with the next chunk
    const mergedChunks: string[] = [];
    for (let i = 0; i < finalChunks.length; i++) {
        const chunk = finalChunks[i];
        const isHeaderOnly =
            /^#{1,6}\s+.+$/.test(chunk.trim()) &&
            chunk.trim().split('\n').length === 1;

        if (isHeaderOnly && i < finalChunks.length - 1) {
            // Merge header with the next chunk
            const nextChunk = finalChunks[i + 1];
